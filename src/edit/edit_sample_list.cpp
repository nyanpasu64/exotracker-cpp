...
// TODO remove this and replace with sample import?
// Should GUI construct a Sample object, or only a subset (skipping tuning)?
...
std::tuple<MaybeEditBox, SampleIndex> try_remove_sample(
    ...
    SampleIndex begin_idx = EXPR(
        ...
        // We're removing the last sample. Find the new last sample.
        for (size_t i = sample_idx; i--; ) {
            if (doc.samples[i].has_value()) {
                return (SampleIndex) i;
            }
        }
        // There are no samples left. Keep the sample as-is.
        // (This differs from FamiTracker which sets the new sample to 0.)
        return sample_idx;
    );
...
struct SwapSamples {
    ...
    void apply_swap(Document & doc) {
        ...
        // TODO tell synth that samples swapped?
    }
