...
// # Playback control methods
    ...
    /// OverallSynth is responsible for calling tick_sequencer() or run_driver() during
    /// playback, and stop_playback() once followed by run_driver() when not playing.
    virtual void seek(doc::Document const& document, doc::TickT time) = 0;
...
// and stops playing when stop_playback() is called.
    ...
    /// Document length, blocks, and/or events are changed.
    /// Keeps real time in ticks, recomputes position in event list.
    /// If real time is past the end of the document, the next tick will loop.
    /// If not playing, ignored.
    virtual void doc_edited(doc::Document const& document) = 0;

// # Driver methods

    /// Reset driver and synth state. Called before seek() whenever playback begins.
    /// You are required to call tick_sequencer() or run_driver() afterwards on the
    /// same tick.
