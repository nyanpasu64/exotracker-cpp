...
At a given tempo, all beats are assumed to be the same length, an integer number of ticks. In both NSF export and PC preview, exotracker converts all note positions from beat fractions into two parts, a non-negative integer part and a "fraction less than 1" part. During NSF export, each fractional part, or possibly (beat fraction, tick offset) tuple, will be converted to a unique integer index. For each speed/tempo/groove used in the song, the NSF file will store an array from integer indexes to "fractional beats at this tempo, rounded to the nearest tick" (possibly with user customization). Then during playback, the NSF driver takes the current tempo and each note's integer index, and looks in the array to find the note's delay relative to the beginning of the beat.
